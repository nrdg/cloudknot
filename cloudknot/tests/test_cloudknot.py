...
def test_transform_data():
    ...
    x1, y1, n1 = ck.transform_data(ortho)
    x2, y2, n2 = ck.transform_data(op.join(data_path, 'ortho.csv'))
    ...
    my_x, my_y, my_n = ck.transform_data(my_data)
...
def test_opt_err_func():
    ...
    my_y = my_x
    my_err = ck.opt_err_func(my_params, my_x, my_y, my_silly_func)
    ...
    my_err = ck.opt_err_func(my_params, my_x, my_y, not_so_silly_func)
    # Since x and y are equal, the error is zero:
    npt.assert_equal(my_err, np.zeros(my_x.shape[0]))
...
def test_Model():
    ...
    target_mu = 0.5
    target_sigma = 1
    target_y = ck.cumgauss(x, target_mu, target_sigma)
...
def test_params_regression():
    ...
    model = ck.Model()
    ortho_x, ortho_y, ortho_n = ck.transform_data(op.join(data_path,
                                                          'ortho.csv'))

    para_x, para_y, para_n = ck.transform_data(op.join(data_path,
                                                       'para.csv'))
