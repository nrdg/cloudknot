...
class Nest(object):
    def __init__(self, name='default', batch_service_role_name=None,
        ...
        if self._nest_name in CONFIG.sections():
            # Nest exists, check that user did not provide any resource names
            ...
            try:
                self._batch_service_role = aws.iam.IamRole(
                    name=CONFIG[self._nest_name]['batch service role']
                )
            ...
            try:
                self._ecs_instance_role = aws.iam.IamRole(
                    name=CONFIG[self._nest_name]['ecs instance role']
                )
            except aws.ResourceDoesNotExistException:
                self._ecs_instance_role = aws.iam.IamRole(
                    name=CONFIG[self._nest_name]['ecs instance role'],
            ...
            try:
                self._spot_fleet_role = aws.iam.IamRole(
                    name=CONFIG[self._nest_name]['spot fleet role']
                )
            except aws.ResourceDoesNotExistException:
                self._spot_fleet_role = aws.iam.IamRole(
                    name=CONFIG[self._nest_name]['spot fleet role'],
            ...
            self._vpc = aws.ec2.Vpc(vpc_id=CONFIG[self._nest_name]['vpc'])

            self._security_group = aws.ec2.SecurityGroup(
                security_group_id=CONFIG[self._nest_name]['security group']
            )
        else:
            # Nest doesn't exist, create it
            ...
            # Save the new nest resources in config object
            CONFIG[self._nest_name] = {
...
class Pipeline(object):
    def __init__(self, nest,
        ...
        self._compute_environment = aws.batch.ComputeEnvironment(
            ...
            batch_service_role=self._nest.batch_service_role,
            instance_role=self._nest.ecs_instance_role,
            vpc=self._nest.vpc,
            security_group=self._nest.security_group,
            desired_vcpus=cpus
        )
    ...
    nest = property(operator.attrgetter('_nest'))
